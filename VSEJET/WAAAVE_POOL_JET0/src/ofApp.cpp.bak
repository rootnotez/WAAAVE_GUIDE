...
//new dummy texture just to have the shader in hd
ofTexture dummyTex;



bool scaleswitch=1;
const int framebufferLength=240;
...
//0 is sd aspect ratio and crops in the y direction, centering video
//use definitely with all of the VSERPI devices 
//and anything else doing like 480i/p over hdmi
//1 is hd and should just fill the screen
...
//framebufferLength is the total number of frames that will be stored. 
...
void ofApp::setup() {
	...
	//ofToggleFullscreen();
	ofHideCursor();
	cameraSetup(widthInput, heightInput);	
    allocateFbo(widthHD,heightHD);
    clearFbo();
	p_lockClear();
	shader_mixer.load("shadersGL4/shader_mixer");
	shaderSharpen.load("shadersGL4/shaderSharpen");
	midiSetup();
...
void ofApp::draw() {
	...
	//input aspect ratios and scaling
	//we ADD a logic here for sd pillarbox vs sd fullscreen?
	
	
	
	
	shader_mixer.setUniformTexture("cam1", aspect_fix_fbo.getTexture(),1);


	
	//send the textures
	shader_mixer.setUniformTexture("fb", pastFrames[d_delay].getTexture(),2);
	
	if(wet_dry_switch==1){
		shader_mixer.setUniformTexture("temporalFilter", pastFrames[(abs(framedelayoffset-framebufferLength)-1)%framebufferLength].getTexture(),3);
	}
	if(wet_dry_switch==0){
		shader_mixer.setUniformTexture("temporalFilter", dry_framebuffer.getTexture(),3);
	}	
	//send the continuous variables
	//heres a test of thresholding.  add this into GW updates along with wipes...
	shader_mixer.setUniform1i("keyEdgeSwitch",0);
                  ...
                  if(control_switch==0){
						if(p_lock_0_switch==1){
							...
							if(midiActiveFloat[3]==1){
								...
								if(  ((message.value-MIDI_MAGIC)/MIDI_MAGIC)<-.99){
									p_lock[3][p_lock_increment]=0.0;
								}
							}
						}
                    }
