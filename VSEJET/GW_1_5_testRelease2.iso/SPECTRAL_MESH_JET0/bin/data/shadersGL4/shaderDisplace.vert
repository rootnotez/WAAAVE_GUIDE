...
float oscillate(float theta,int shape,int xyz){
    ...
    if(shape==3){
        ...
        if(xyz==0){osc=2.0*(texture(x_noise_image,texCoordVarying*.5).r-.5);}
        if(xyz==1){osc=2.0*(texture(y_noise_image,texCoordVarying*.5).r-.5);}
        if(xyz==2){osc=2.0*(texture(z_noise_image,texCoordVarying*.5).r-.5);}
    }
    return osc;
}
//--------------------------------------------------------------------
