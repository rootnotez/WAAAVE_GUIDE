...
void ofApp::draw() {
    ...
    if(hdmi_aspect_ratio_switch==0){
		time_displaced_texture.draw(0,0,720,576);
	}
	if(hdmi_aspect_ratio_switch==1){
		time_displaced_texture.draw(0,0,720*1.34,576*1.34);
	}	
    ...
    if( (framedelayoffset<=d_freeze_upper) && (framedelayoffset>=d_freeze_lower) ){
	    ...
	    */
		if(inputswitch==1){
			input1.update();
			if(hdmi_aspect_ratio_switch==0){
				pastFrames_img[abs(fbob-framedelayoffset)-1].setFromPixels(input1.getPixels());
			}
			if(hdmi_aspect_ratio_switch==1){
