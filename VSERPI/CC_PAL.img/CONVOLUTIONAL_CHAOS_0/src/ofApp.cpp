...
//control1 comes from midi
float control1[controlSize];
//control2 is midi * 
float control2[controlSize];
string controlNames[controlSize];
...
void ofApp::setup() {
	...
	shaderConvolution.load("shadersES2/shaderConvolution");
	midiSetup();
...
void ofApp::draw() {
	...
	float weight=1.5;
	for(int i=0;i<8;i++){
		control2[i]=control1[i]*weight;
	}
	float dist=5.0;
	for(int i=8;i<controlSize;i++){
		control2[i]=(1.0f+control1[i]);
	}
	framebuffer0.begin();
	shaderConvolution.begin();
	...
	shaderConvolution.setUniformTexture("fb",framebuffer1.getTexture(),1);
	for(int i=0;i<controlSize;i++){
		shaderConvolution.setUniform1f(controlNames[i],control2[i]);
		//cout<<"controlNames["<<i<<"] = "<<controlNames[i]<<endl;
		//cout<<"control2["<<i<<"[ = "<<control2[i]<<endl;
	}	
	shaderConvolution.end();
	framebuffer0.end();
	
	framebuffer0.draw(0,0,720,576);
